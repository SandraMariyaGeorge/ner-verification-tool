...
export default function ProjectEntityPage({ params }) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const data = await getEntityOccurrences(word.trim(), projectId);
  ...
  const handleApply = async () => {
    ...
    try {
      await bulkUpdateEntity(word.trim(), tag, projectId);
  ...
  return (
    <div className="grid">
      <section className="card">
        <h2>Entity Bulk Correction - Project {projectId}</h2>
