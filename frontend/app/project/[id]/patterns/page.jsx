...
export default function ProjectPatternPage({ params }) {
  ...
  const handleDetect = async () => {
    setLoading(true);
    try {
      const data = await getPatternErrors(projectId);
  ...
  const handleApplyFix = async (item) => {
    try {
      await applyPatternFix(item, projectId);
  ...
  return (
    <div className="grid">
      <section className="card">
        <h2>Pattern Error Detection - Project {projectId}</h2>
