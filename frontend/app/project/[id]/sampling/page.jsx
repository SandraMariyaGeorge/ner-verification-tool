...
export default function ProjectSamplingPage({ params }) {
  ...
  const handleGenerateSample = async (size) => {
    setLoading(true);
    try {
      const [sample, preview] = await Promise.all([getSample(size, projectId), getPreview(25, projectId)]);
  ...
  const handleTagChange = async (token, newTag) => {
    try {
      await updateSingleTag(token, newTag, projectId);
      ...
      const rows = await getEntityOccurrences(token.word, projectId);
  ...
  const handleApplyToAll = async () => {
    try {
      await bulkUpdateEntity(bulkState.word, bulkState.newTag, projectId);
  ...
  return (
    <div className="grid">
      <section className="card">
        <h2>Sampling - Project {projectId}</h2>
